...
import {
  colorConfig,
  KuanticTheme,
} from './../../kuantic-color-picker/KuanticTheme'
...
export default {
  ...
  computed: {
    backgroundImage () {
      const theme = colorConfig[KuanticTheme[this.theme]]
      const value = this.animatedValue
      const backgroundTheme = colorConfig[KuanticTheme[this.backgroundTheme]]
    ...
    circleBarStyle () {
      return {
        backgroundColor: colorConfig[KuanticTheme[this.backgroundTheme]],
        color: colorConfig[KuanticTheme[this.theme]],
      }
    },
  },
}
